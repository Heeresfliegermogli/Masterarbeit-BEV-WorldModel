#!/bin/bash
# =============================================================================
# sbatch_20_dump_baseline.sh — Task 20: Vorhersage-Dump det_baseline (Val)
# =============================================================================
# 1 GPU, ohne shm-Staging (mmap BeeGFS). Dump -> predictions/task20/dump_det_baseline
# (5743 Vorhersagen + 276 Real-Fill, Roh-Skala fp16). IMMER PLAIN. NIE --export.
# =============================================================================
#SBATCH --job-name=dump20b
#SBATCH --gres=gpu:a100:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=96G
#SBATCH --time=03:00:00
#SBATCH --output=logs/dump20b_%j.out
...
python -u eval_dump_latents.py --config config_det_baseline_eval.yaml \
    ...
    --phase cell --output_dir predictions/task20/dump_det_baseline
echo "[done] $(ls predictions/task20/dump_det_baseline | wc -l) Dateien (erwartet 6019)"
