...
# =============================================================================
#SBATCH --job-name=sweep22
#SBATCH --gres=gpu:a100:5
#SBATCH --cpus-per-task=110
#SBATCH --mem=500G
#SBATCH --time=16:00:00
#SBATCH --output=logs/sweep22_%j.out
...
LABELS=(rare_w2 rare_w4 rare_w8 samp_rare samp_dyn)
declare -A CFG=()
...
echo "[stage] $(date +%T)"
python -u "$SHM_STAGING" --stage --config "${CFG[rare_w2]}" || { echo "FEHLER: Pre-Staging."; exit 1; }
...
for g in 0 1 2 3 4; do wait "${PIDS[$g]}" || { echo "[train] FEHLGESCHLAGEN ${LABELS[$g]}" >&2; FAIL=1; }; done
...
for g in 0 1 2 3 4; do [ -n "${IPIDS[$g]}" ] && { wait "${IPIDS[$g]}" || echo "[inference] FEHL ${LABELS[$g]}" >&2; }; done
